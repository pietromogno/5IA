 ...
 * @author Davide
 */
public class GuiAggiungiPartecipanteGruppo extends javax.swing.JFrame {

    private GuiAggiungiGruppo guiAggiungiGruppo;

    public GuiAggiungiPartecipanteGruppo(Client client, GuiAggiungiGruppo guiAggiungiGruppo) {
        setLookAndFeel();
        initComponents();
        this.guiAggiungiGruppo = guiAggiungiGruppo;
        cmbBoxContatti.removeAllItems();
        for (Contatto contatto : client.getListaContatti()) {
            cmbBoxContatti.addItem(contatto);
        }
        this.setTitle(client.getAccount().getUsername() + " - Aggiungi Partecipante Gruppo");
    ...
    private void initComponents() {
        ...
        btnConfermaScelta.setText("Aggiungi Partecipante");
    ...
    private void btnConfermaSceltaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnConfermaSceltaActionPerformed
        if (cmbBoxContatti.getItemCount() > 0) {//se c'è qualcosa dentro allora faccio azione di selezione
            ...
            guiAggiungiGruppo.addPartecipante(contattoPerConversazione);
            this.dispose();
        }
